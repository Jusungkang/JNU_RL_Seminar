...
from Proposed_ResNet_v1_RFIBE import resnet_DPN, resnet152, resnet50
...
class Mario:
    def __init__(self, state_dim, action_dim, save_dir):
        ...
        self.net = resnet50(self.state_dim, self.action_dim).float()
    ...
    def td_target(self, reward, next_state, done):
        ...
        # self.gamma.to("cpu")
        next_Q_cpu = next_Q.to("cpu")

        return_reward = (reward + (1 - done.float()) * self.gamma * next_Q_cpu).float()

        return return_reward.to('cuda')
...
class REWARD :
    def __init__(self, init_info):
        self._x_pos = init_info['x_pos']
        self._y_pos = init_info['y_pos']
        self._score = init_info['score']

    def Reward_cal(self, reward, info_next, done):
        reward_dis = np.float32((info_next['x_pos'] - self._x_pos) / 30)
        reward_score = np.float32((info_next['score'] -  self._score) / 50)

        reward_time = -1

        reward_total = reward_dis + reward_score + reward_time + reward
        ...
        self._x_pos = info_next['x_pos']
        self._y_pos = info_next['y_pos']
        self._score = info_next['score']
...
calculation_reward = REWARD(info)
...
mario.net.state_dict(torch.load("./checkpoints/SuperMario_w_DQN_Res/mario_net_" + str(Target_Net_Number) + ".chkpt", map_location=torch.device('cpu')))


## Data Logger 호출
logger = MetricLogger(save_dir)

## 권장사항 => Episodes 적어도 40000회 이상 권장. (feat. Right_Only)
# episodes = 50000
...
for e in range(episodes):
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            # reward_cal = calculation_reward.Reward_cal(reward, info, done)
            #
            # print(reward)
            # print(reward_cal)
            ...
            # mario.cache(state, next_state, action, reward_cal, done)
